...
export const episodeAnalysisSchema = z.object({
  ...
  tileCandidateFrameIndices: z
    ...
    .optional()
    .describe(
      "从上面展示的编号帧（Frame N，覆盖全片的稀疏采样）里挑出帧序号，供后续生成桥接内容时作为人物形象/场景的视觉素材候选参考。" +
        "优先级：先保证上面角色列表里的每一个主要角色，都至少有一帧能清晰看到其外貌（长相、发型、服装）的画面被选中，" +
        "同一个角色有多个镜头时选画面最清晰、最能代表其形象的那帧；主要角色都覆盖到之后，名额还有富余再补充有代表性的场景镜头。" +
        "模糊、转场、看不清人物的空镜头不要选，找不到合适的就返回空数组。只从 Frame N 编号的帧里选，不要参考结尾部分的 Tail frame N。",
    ),
});
